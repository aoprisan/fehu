...
use fehu_webapp::journal::{self, Command, JournalEntry, Principal};
...
async fn the_engine_step_is_journaled_and_replay_never_reads_the_clock() {
    ...
    fehu_webapp::engine::advance_to(&app, first).await;
    fehu_webapp::engine::advance_to(&app, second).await;
...
async fn stopping_at_any_point_comes_back_at_that_point_and_no_reward_is_paid_twice() {
    ...
    for i in 0..12u64 {
        let r = match i % 4 {
            ...
            2 => {
                fehu_webapp::engine::advance_to(&app, fehu::Timestamp(NOW_MS + 1_000 * i as i64))
                    .await;
                continue;
            }
            _ => {
