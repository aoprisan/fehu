...
> Paths in this document have been updated for the workspace layout:
> the library is `crates/fehu/` and the server, once `webapp/` and the
> package `fehu-webapp`, is `crates/fehu-economy/`. Nothing else about
...
This plan is grounded in the current source (the server's save version 5). It is
...
Evidence is from this checkout. Line references are to `crates/fehu-economy/src/` unless
...
Baseline: `cargo test --all-features` and `cargo test -p fehu-economy` pass
...
Changes to `crates/fehu-economy/src/account.rs` once the ledger exists. `Account` keeps
...
```rust
// crates/fehu-economy/src/journal.rs (sketch)
...
and every existing DTO in `crates/fehu-economy/ui/src/types.ts` and the contract test
...
short-lived stream ticket. That changes `crates/fehu-economy/ui/src/stream.ts` and needs
...
**The baseline was not green.** `cargo clippy -p fehu-economy --all-targets
...
Fixed as part of milestone 0. The library and server test suites did pass.
...
Done. `crates/fehu-economy/src/journal.rs` holds the command model, the file, the
idempotency index and the single apply path; `crates/fehu-economy/tests/journal.rs` holds
...
Done. `src/exchange.rs` gained the switch, `crates/fehu-economy/src/symbol.rs` the asset
kind, `crates/fehu-economy/src/catalog.rs` and `crates/fehu-economy/src/npc.rs` are new, and
`crates/fehu-economy/tests/goods.rs`, `crates/fehu-economy/tests/npc.rs` and
`crates/fehu-economy/tests/economy_goods.rs` hold the acceptance. Six things are worth
recording.
...
Done. `crates/fehu-economy/src/jobs.rs`, `crates/fehu-economy/src/rewards.rs` and `crates/fehu-economy/src/world.rs`
are new; `crates/fehu-economy/tests/jobs.rs`, `crates/fehu-economy/tests/rewards.rs` and
`crates/fehu-economy/tests/economy_jobs.rs` hold the acceptance; the UI gained one panel
...
Done, and narrower than the row it was written into. `crates/fehu-economy/src/outbox.rs`
is new; `limit.rs` gained an `Admission`; `crates/fehu-economy/tests/outbox.rs` and
`crates/fehu-economy/tests/load.rs` hold the acceptance; the save format went to version
...
second persistence model to a server whose whole persistence story fits in
...
when is p95 latency, and `crates/fehu-economy/tests/load.rs` asserts one — but a p95
...
(`crates/fehu-economy/ui/src/panels/ops.ts`); `crates/fehu-economy/tests/overview.rs` holds the
...
lives in the library rather than in the server for the same reason the
...
Done. `crates/fehu-economy/src/service.rs` is new; `account.rs` gained the player
mapping, `market.rs` the registry behind the published directory, `api.rs`
the `Trusted<Scope>` extractor and five routes; `crates/fehu-economy/tests/service.rs`
...
cargo test -p fehu-economy      # server: all suites pass, 0 failed
```
...
cargo clippy -p fehu-economy --all-targets -- -D warnings    # clean
...
cargo test -p fehu-economy                                   # 298 passed, 0 failed
cd crates/fehu-economy/ui && npm ci && npm run build                     # bundle rebuilt and committed
```
...
totals. The server gained `tests/overview.rs` (8) — the overview is the
...
The 288 server tests that predate the milestone pass unchanged. The UI
...
cargo test -p fehu-economy                                   # 288 passed, 0 failed
```

The server gained `tests/service.rs` (14) and `service.rs`'s own unit tests
...
The 265 server tests that predate the milestone pass unchanged, with one
...
cargo test --all-features                                   # 103 passed, 0 failed
cargo test --no-default-features --tests                    #  96 passed, 0 failed
cargo test -p fehu-economy                                   # 265 passed, 0 failed
```

The server gained `tests/outbox.rs` (14) and `tests/load.rs` (8), and the
...
cargo test --all-features                                   # 103 passed, 0 failed
cargo test --no-default-features --tests                    #  96 passed, 0 failed
cargo test -p fehu-economy                                   # 228 passed, 0 failed
cargo build --release --target wasm32-unknown-unknown …     # clean, both feature sets
cd crates/fehu-economy/ui && npm ci && npm run build                     # bundle rebuilt and committed
```

Three new suites. `crates/fehu-economy/tests/jobs.rs`: a recipe that names only listed
...
`crates/fehu-economy/tests/rewards.rs`: a budget filled out of treasury and never minted,
...
`crates/fehu-economy/tests/npc.rs`: a world with no synthetic liquidity, seeded, quoting
both sides of every book and owing nobody anything.

`crates/fehu-economy/tests/economy_jobs.rs` is the milestone's own acceptance: the
...
The 195 server tests that predate the milestone are unchanged and pass, apart
...
cargo test --all-features                                   # 103 passed, 0 failed
cargo test --no-default-features --tests                    #  96 passed, 0 failed
cargo test -p fehu-economy                                   # 179 passed, 0 failed
```

Three new suites. `crates/fehu-economy/tests/goods.rs`: a good listed holding nothing and
...
with goods in it coming back whole. `crates/fehu-economy/tests/npc.rs`: a merchant funded
...
`crates/fehu-economy/tests/economy_goods.rs` is the milestone's own acceptance: every
...
The 145 server tests that predate the milestone are unchanged and pass.
...
cargo test -p fehu-economy                                   # 156 passed, 0 failed
```

New suite: `crates/fehu-economy/tests/journal.rs` — a restart with no snapshot of the work
...
The other 145 server tests are unchanged and pass, which is the useful
...
cargo clippy --all-targets --all-features -- -D warnings    # clean
cargo clippy --all-targets --no-default-features -- -D warnings
cargo clippy -p fehu-economy --all-targets -- -D warnings    # clean (was red)
cargo build --all-features / --no-default-features / +serde # clean
cargo test --all-features                                   # 102 passed, 0 failed
cargo test --no-default-features --tests                    #  95 passed, 0 failed
cargo test -p fehu-economy                                   # 136 passed, 0 failed
cargo build --release --target wasm32-unknown-unknown …     # clean, both feature sets
cd crates/fehu-economy/ui && npm ci && npm run build                     # bundle unchanged
```
...
`no_std` paths) and `crates/fehu-economy/tests/economy.rs` (conservation across
