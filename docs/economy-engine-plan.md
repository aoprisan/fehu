...
| Issuance permissions | `api.rs::open_account`, `deposit`, `trader_deposit`, and trader creation allow player-controlled funding; account status changes require ownership | Remove funding from player commands; separate operator freezes from voluntary account closure |
...
GAME is the unit of account and quote currency. Preserve integer cents: 1 GAME = 100 cents. Do not introduce an 18-decimal denomination merely to resemble crypto. Use `CurrencyId` now, but implement one quote currency initially. Preserve signed integer posting deltas, nonnegative wallet balances, and checked wide intermediate multiplication. Encode monetary integers as decimal strings in the new API so JavaScript clients cannot lose precision; update TypeScript DTOs accordingly.
...
4. Commit command result, economic records, outbox events and sufficient versioned state delta to reconstruct the exact candidate in one database transaction.
...
Symbol actors may calculate candidate transitions but must not expose them as committed state. Start with the economy actor owning authoritative mutable books and symbol actors serving committed projections; parallelize preparation later if needed. This avoids distributed commit between today's independently mutating symbol tasks. Engine ticks, stop triggers, expiries, NPC decisions and production completions must use the same pipeline. Publish no fill before its settlement commits.

Checkpoint plus ordered committed deltas must restore books, holds, simulator/RNG state, jobs and balances at one sequence. Record policy changes and tick order, so replay never depends on current wall time or a changed recipe. Preserve the original simulation mode and its deterministic native/Wasm tests.

## Game interactions and API

Use `/api/v1/economy` for the authoritative contract; retain legacy APIs only in an explicitly enabled demo mode. Require authenticated service credentials in economy mode and fail startup if authority/storage configuration is missing. Never put operator credentials in the browser. Scope service principals to provisioning, rewards, inventory grants, events or administration; derive player identity from authentication rather than trusting a body field.
...
Deliver phases in dependency order. Phases 1–3 establish the token service; 4–6 complete the requested game economy. Do not call the wallet-only milestone a complete economy engine. Estimate effort after the staged matching/storage prototype establishes how much exchange state can be reused; avoid committing to throughput or dates without measurement.
