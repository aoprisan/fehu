...
/// history — for the reason the `fehu-economy` server gives for its own
/// metrics: a running total is exact, costs one add on a path that already
/// does far more work, and anything that wants a rate can subtract two
/// readings. The numbers only ever grow, so two readings and the time
/// between them are all a rate needs.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
pub struct Flows {
