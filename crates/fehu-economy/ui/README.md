...
The front end for the economy server in `crates/fehu-economy/`. TypeScript,
built with Vite, no UI framework.
...
`../static/` is **committed build output**, embedded into the binary by
`include_str!` in `../src/api.rs`. That keeps `cargo run -p fehu-economy` a
...
`src/types.ts` mirrors the `#[derive(Serialize)]` types in `../src/` and
the parts of the `fehu` crate that cross the wire. It is hand-written, and
`../tests/contract.rs` pins the exact JSON key set of every response, so
renaming a field in Rust fails `cargo test -p fehu-economy` with a message
